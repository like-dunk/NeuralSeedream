...
class APIClient:
    ...
    def create_task(
        ...
        if len(image_urls) > MAX_INPUT_IMAGES:
            logger.warning(f"输入图片数量超过限制，截取前{MAX_INPUT_IMAGES}张")
        ...
        logger.debug(f"创建任务: prompt长度={len(prompt)}, 图片数={len(image_urls)}")
    ...
    def wait_for_result(self, task_id: str, log_prefix: str = "") -> TaskResult:
        ...
        while True:
            ...
            if status == "success":
                ...
                logger.debug(f"{log_prefix} 生成耗时 {int(elapsed)} 秒")
                return TaskResult(
                    task_id=task_id,
                    status="success",
                ...
                if current_time >= last_log_time + 10:
                    logger.info(f"{log_prefix} ⏳ 生成中... {current_time}秒")
